...
class Getcoins extends Component
{
    ...
    {   
        ...
        return view('livewire.components.okx.getcoins');
    }
    ...
    public function todotodo() {
        ...
        $berns = DB::select('SELECT `type`, coin, price, position FROM `precio_okex` WHERE fiat = ?', [$this->fiat]);
        ...
        $berns = DB::select('SELECT coin, `price` as `price_usdt` FROM `price_usdt_okex`');
    ...
    public function getCoins($fiat) {
      //  dd($this->coinpos);
        $this->fiat = $fiat;
        $berns = DB::select('SELECT `type`,coin,price, position FROM `precio_okex` where fiat = ?',[$fiat]);
        ...
        $posiciones = DB::select("SELECT * FROM `precio_okex` where fiat = ? and `type` = 'buy'", [$this->fiat]);
        ...
        $posiciones = DB::select("SELECT * FROM `precio_okex` where fiat = ? and `type` = 'sell'", [$this->fiat]);
