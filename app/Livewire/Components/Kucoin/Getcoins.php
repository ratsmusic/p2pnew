...
class Getcoins extends Component
{
    ...
    {   
        ...
        return view('livewire.components.kucoin.getcoins');
    }
    ...
    public function todotodo() {
        ...
        $berns = DB::select('SELECT `type`, coin, price, position FROM `precio_kucoin` WHERE fiat = ?', [$this->fiat]);
        ...
        $berns = DB::select('SELECT coin, `price` as `price_usdt` FROM `price_usdt_kucoin`');
        ...
        foreach ($this->monedas as $coin => &$positions) {
            foreach (['buy', 'sell'] as $type) {
                foreach ($positions[$type] as $position => &$moneda) {
                    ...
                    //if($moneda['symbol_price']==0) dd($moneda);
                    if ($type == 'buy') {
    ...
    public function getCoins($fiat) {
      //  dd($this->coinpos);
        $this->fiat = $fiat;
        $berns = DB::select('SELECT `type`,coin,price, position FROM `precio_kucoin` where fiat = ?',[$fiat]);
        ...
        $posiciones = DB::select("SELECT * FROM `precio_kucoin` where fiat = ? and `type` = 'buy'", [$this->fiat]);
        ...
        $posiciones = DB::select("SELECT * FROM `precio_kucoin` where fiat = ? and `type` = 'sell'", [$this->fiat]);
