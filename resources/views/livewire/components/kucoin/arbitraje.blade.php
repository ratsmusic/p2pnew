                                                ...
                                                @foreach ($principiante_buy as $index => $row)
                                                    <tr>
                                                        <td class="text-center" style="white-space: nowrap;">
                                                            <img src="{{ asset('img/coin/' . strtolower($row['coin']) . '.svg') }}" alt="{{ $row['coin'] }}" style="height: 20px; margin-right: 5px;">
                                                            {{$row['coin']}}
                                                            <select class="form-select form-select-sm d-inline-block ms-2" style="width: auto;" wire:change="actualizarPos2('{{$row['coin']}}',$event.target.value)" wire:model='coinpos2.{{$row['coin']}}' wire:key="{{$row['coin']}}-7">
                                                                @foreach($posiciones2[$row['coin']] as $pos)
                                                                    <option value="{{ $pos }}" {{$pos == 3 ? 'selected' : ''}}>{{ $pos }}</option>
                                                                @endforeach
                                                            </select>
                                                ...
                                                @foreach ($principiante_sell as $index => $row)
                                                    <tr>
                                                        <td class="text-center" style="white-space: nowrap;">
                                                            <img src="{{ asset('img/coin/' . strtolower($row['coin']) . '.svg') }}" alt="{{ $row['coin'] }}" style="height: 20px; margin-right: 5px;">
                                                            {{$row['coin']}}
                                                            <select class="form-select form-select-sm d-inline-block ms-2" style="width: auto;" wire:model='coinpos.{{$row['coin']}}' wire:change="actualizarPos('{{$row['coin']}}',$event.target.value)" wire:key="{{$row['coin']}}-9">
